...
- **A single endpoint can serve more than one entity type, and its name doesn't disclose this.** `GetCandidateDetails` returns Committee records as well as Candidate records, depending on a request parameter, despite its name. This pattern, a shared endpoint underlying what looks like several distinct pages, recurs elsewhere and is worth checking for in any endpoint before assuming it's page-specific.
